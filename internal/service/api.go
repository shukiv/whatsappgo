...
var apiMethods = []MethodDescription{
	...
	method("profile.export", "Write this account to a portable archive; it carries the device credentials, so move it like a password and run the profile on one machine only. deactivate retires this copy once the archive exists, which is what keeps one account from running in two places", true,
		map[string]any{"path": "/tmp/israeli.wagprofile", "include_media": false, "deactivate": false}),
