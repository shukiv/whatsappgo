...
A reliability release. It fixes sixteen confirmed defects found by reviewing the
desktop and the daemon, and adds no new features. It does not imply full
WhatsApp feature parity.

## Highlights

- Sends are refused for addresses that are not conversations, so answering an
  incoming event cannot publish a status update to your contacts.
- Exporting a chat no longer replaces a file that is already at that path.
- A notification the desktop service refuses is handed back to the window
  instead of disappearing, and notification text is escaped for servers that
  parse markup.
- Incoming-call and security-code alerts can no longer be crowded out by a run
  of status updates.
- Dialogs, drafts, clipboard copies, media pages and starred results survive
  stale responses, chat changes and account switches.
- Read receipts are deferred while the conversation is not on screen.

## Desktop reliability fixes

### Ten confirmed bugs fixed in the desktop
...
Six further defects found while reviewing the application, with their regressions:
...
- **Exporting a chat:** `chat.export` refuses to replace a file that already
  exists unless the caller passes `replace`. The desktop passes it because its
  save dialog has already asked. A script that exported to an existing path
  used to overwrite it without a word.
...
- Starred-message search covers the latest 100 loaded starred messages.
- Calendar navigation searches local history, not unloaded server history.
- Older-media pagination, full-library sorting and filtering, bulk local
  deletion, unstar-all, and video picture-in-picture remain follow-up work.
- Voice and video calls are not supported.
- The notification fallback needs a system tray, and on Linux that tray balloon
  is drawn by the same notification service that refused the first attempt.

## Installation

Download the artifact for your platform and verify it against `SHA256SUMS`
before installation. Builds are unsigned: Windows SmartScreen may warn, and
macOS may require approval in System Settings. The release remains a draft
until the packaged binaries have been reviewed.
